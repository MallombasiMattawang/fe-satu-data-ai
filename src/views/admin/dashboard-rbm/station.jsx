...
export default function DashboardStation() {
  //title page
  document.title = "Dashboard - Station";
  ...
  const handleSubmit = (e) => {
    ...
    window.location.href = `/admin/patroli-rbm-station/${station}?${queryString}`;
  };
  ...
  const fetchPatroli = async () => {
    try {
      const response = await Api.get(`/api/admin/maps-rbm?station=${station}`, {
        headers: {
  ...
  useEffect(() => {
    //fetch api
    Api.get(`/api/admin/patroli-rbm-station/${station}`, {
      params: {
    ...
    <LayoutAdmin>
      <main>
        <div className="container-fluid px-4 mt-5">
          <h2 className="mb-4"><i className="fa fa-dashboard"></i> Dashboard RBM {station}</h2>
          <div className="row">
            ...
            <div className="col-xl-6 col-md-6">
              <div className="card mb-4 border-0 shadow-sm" style={{ height: "550px" }}>
                <div className="card-header">
                  <h6>Heat Map Patroli RBM {station} {year ? `Tahun ${year}` : " Semua Tahun"}</h6>
                </div>
